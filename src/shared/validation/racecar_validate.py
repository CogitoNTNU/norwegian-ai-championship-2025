"""
Validation script for the Race Car Control leaderboard.
"""
...
def submit_validation() -> None:
    """Submit the Race Car Control API for validation."""
    env_vars = load_env()

    if not validate_env_vars(env_vars):
        return

    # Use the specific service URL for race car
    service_url = env_vars.get("RACE_CAR_SERVICE_URL", "http://0.0.0.0:9052")

    url = "https://cases.ainm.no/api/v1/usecases/race-car/validate/queue"
    headers = {"x-token": env_vars["EVAL_API_TOKEN"]}
    data = {"url": f"{service_url}/predict"}

    print_status(
        f"🚀 Submitting validation for Race Car Control API: {service_url}/predict",
        "info",
    )
    ...
    if not success:
        ...
        if "--wait" in sys.argv:
            wait_for_completion("race-car", uuid, env_vars["EVAL_API_TOKEN"])


def check_status(uuid: str) -> None:
    """Check the status of a Race Car Control validation attempt."""
    env_vars = load_env()

    if not validate_env_vars(env_vars):
        return

    url = f"https://cases.ainm.no/api/v1/usecases/race-car/validate/queue/{uuid}"
    headers = {"x-token": env_vars["EVAL_API_TOKEN"]}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate Race Car model")
    parser.add_argument(
        "--model", default="baseline", help="Model variant to use (baseline, rl)"
    )
    parser.add_argument(
    ...
    os.environ["RACECAR_MODEL_VARIANT"] = args.model

    print(f"🎯 Using Race Car model variant: {args.model}")
    submit_validation()
