"""
Validation script for the Emergency Healthcare RAG leaderboard.
"""
...
def submit_validation() -> None:
    """Submit the Emergency Healthcare RAG API for validation."""
    env_vars = load_env()

    if not validate_env_vars(env_vars):
        return

    # Use the specific service URL for emergency healthcare RAG
    service_url = env_vars.get(
    ...
    print_status(
        f"🚀 Submitting validation for Emergency Healthcare RAG API: {service_url}/predict",
        "info",
    )
    ...
    if not success:
        ...
        if "--wait" in sys.argv:
            wait_for_completion(
                "emergency-healthcare-rag", uuid, env_vars["EVAL_API_TOKEN"]
            )


def check_status(uuid: str) -> None:
    """Check the status of an Emergency Healthcare RAG validation attempt."""
    env_vars = load_env()

    if not validate_env_vars(env_vars):
        return

    url = f"https://cases.ainm.no/api/v1/usecases/emergency-healthcare-rag/validate/queue/{uuid}"
    headers = {"x-token": env_vars["EVAL_API_TOKEN"]}
