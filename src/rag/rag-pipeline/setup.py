...
def run_training_evaluation():
    ...
    try:
        ...
        train_statements = (
            base_path / "DM-i-AI-2025/emergency-healthcare-rag/data/train/statements"
        )
        train_answers = (
            base_path / "DM-i-AI-2025/emergency-healthcare-rag/data/train/answers"
        )

        results = pipeline.evaluate_on_training_data(
            str(train_statements), str(train_answers), max_samples=5
        )
...
def main():
    ...
    if success:
        ...
        print("\nNext steps:")
        print(
            "1. Test with: python -c 'from classifier import predict; print(predict(\"test statement\"))'"
        )
