...
def create_radar_chart(template_ragas_scores, output_dir, ragas_metrics):
    ...
    print(f"Radar chart saved to {os.path.join(output_dir, 'ragas_metrics_radar_chart.html')}")
...
def create_bar_charts(template_additional_scores, output_dir):
    ...
    print(f"Bar charts saved to {os.path.join(output_dir, 'additional_metrics_bar_charts.html')}")
...
def get_ranked_templates(template_ragas_scores, template_additional_scores, recall_weight=0.3, precision_weight=0.3, faithfulness_weight=0.4):
    template_scores = {}
    for template_name in template_ragas_scores.keys():
        ragas_scores = template_ragas_scores[template_name]
...
def provide_recommendations(ranked_templates, template_ragas_scores, template_additional_scores, ragas_metrics):
    ...
    print("- Monitor the performance of the RAG template in production and adjust the weights accordingly.")
    print("- Continuously evaluate the RAG template with new test cases to ensure its effectiveness.")

def save_results(template_results, template_ragas_scores, template_additional_scores, ranked_templates, output_dir):
    results = {
                ...
                with open(results_file, 'r') as f:
                    ...
                    for template_name, scores in data["template_ragas_scores"].items():
                        ...
                        summary_data.append({
                            ...
                            "avg_score": np.mean([scores.get("context_recall", 0), scores.get("context_precision", 0), scores.get("answer_relevancy", 0)])
                        })
...
def create_comparison_charts(summary_data, results_dir="results"):
    ...
    metrics_to_plot = ["context_recall", "context_precision", "response_time", "answer_relevancy", "avg_score"]
