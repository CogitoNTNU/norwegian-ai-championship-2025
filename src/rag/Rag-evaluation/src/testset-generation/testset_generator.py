...
async def main():
    ...
    try:
        ...
        json_docs = [Document(page_content=c.get("description", ""), metadata={"name": c.get("name"), "source": "components.json"}) for c in components]
    ...
    # ----------------------------
    persona1 = Persona(name="Maritime UI Developer", role_description="A frontend developer new to maritime applications, building user interfaces for ship bridge systems and needs to understand which components to use for different interaction patterns")

    persona2 = Persona(name="UX Designer for Industrial Systems", role_description="A UX designer working on control interfaces for maritime and industrial equipment, focusing on creating intuitive and accessible user experiences")

    persona3 = Persona(name="Maritime Software Architect", role_description="A senior developer designing the overall structure of maritime software applications and needs to understand component relationships and integration patterns")

    persona4 = Persona(name="Junior Developer Learning OpenBridge", role_description="A junior developer who has been assigned to work with OpenBridge components but is unfamiliar with maritime interface design patterns and component usage")

    persona5 = Persona(name="Product Manager for Maritime Software", role_description="A product manager overseeing maritime software development who needs to understand component capabilities to make informed decisions about feature requirements")

    persona6 = Persona(name="QA Engineer for Maritime Interfaces", role_description="A quality assurance engineer testing maritime user interfaces and needs to understand expected component behaviors and interaction patterns")

    persona7 = Persona(name="Accessibility Specialist", role_description="An accessibility expert ensuring maritime interfaces meet accessibility standards and needs to understand component accessibility features and proper usage")

    persona8 = Persona(name="Integration Developer", role_description="A developer working on integrating OpenBridge components into existing maritime systems and needs detailed technical implementation guidance")

    persona_list = [persona1, persona2, persona3, persona4, persona5, persona6, persona7, persona8]
    ...
    for scenario, result in zip(all_scenarios, all_results):
        ...
        for node in scenario.nodes:
            if node.type == NodeType.CHUNK and hasattr(node, 'parent_id') and node.parent_id:
