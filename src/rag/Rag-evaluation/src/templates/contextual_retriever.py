...
class ContextualRetrieverRAG:
    ...
    def run(self, question: str, reference_contexts: List[str]) -> Dict[str, Union[str, List[str]]]:
        ...
        for i, split in enumerate(splits):
            ...
            contextualized_content = cr_prompt_chain.invoke({'question': 'Please give a short succinct context to situate this chunk within the overall document (the group of 10 chunks) for the purposes of improving search retrieval of the chunk. Answer only with the succinct context and nothing else.'})
        ...
        elif isinstance(result['context'], list) and all(isinstance(doc, Document) for doc in result['context']):
