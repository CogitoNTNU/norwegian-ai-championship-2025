...
def check_multiple_facts(statements: List[str]) -> List[Dict]:
    ...
    for i, statement in enumerate(statements, 1):
        ...
        if original != result['verdict']:
            logger.info(f"Original: {original} -> Final: {result['verdict']} | Topic: {result['topic']}")
        else:
...
def print_detailed_results(results: List[Dict]):
    ...
    logger.info(f"  UNVERIFIABLE: {unverifiable_count} ({100*unverifiable_count/total:.1f}%)")
...
def plot_fact_check_results(results: List[Dict]):
    ...
    for result in results:
        ...
        original_verdict_counts[original_verdict] = original_verdict_counts.get(original_verdict, 0) + 1
    
    if original_verdict_counts:
        axes[0, 0].pie(original_verdict_counts.values(), labels=original_verdict_counts.keys(), autopct='%1.1f%%', startangle=90)
        axes[0, 0].set_title('Original Verdict Distribution\n(Before UNSURE→TRUE Mapping)')
    
    # 2. Relevance score distribution
    relevance_scores = [r.get('avg_relevance_score', 0) for r in results if r.get('avg_relevance_score', 0) > 0]
    if relevance_scores:
...
def main():
    ...
    logger.info("Use the check_fact(statement) function directly or via the API endpoint.")
    logger.info("For testing purposes, you can import and use the functions programmatically.")
