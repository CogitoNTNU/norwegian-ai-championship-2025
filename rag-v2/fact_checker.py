...
def check_fact(statement: str, k: int = 5, model_name: str = "cogito:32b") -> Dict:
    ...
    Args:
        ...
        model_name: Ollama model to use for fact checking (default: cogito:32b)
    ...
    llm = OllamaLLM(
        model=model_name,
        temperature=0,
        base_url="https://11n4k38q0zmgv4-11434.proxy.runpod.net",  # Cloud RunPod instance
    )
...
def check_multiple_facts(
    statements: List[str], k: int = 5, model_name: str = "cogito:32b"
) -> List[Dict]:
    """
    ...
    Args:
        ...
        model_name: Ollama model to use (default: cogito:32b)
    ...
    for i, statement in enumerate(statements, 1):
        ...
        print(
            f"  Verdict: {result['verdict']} | Topic: {result['topic']} | Chunks: {result.get('chunks_retrieved', 'N/A')}"
        )
...
def export_results_to_json(
    results: List[Dict], filename: str = "fact_check_results.json"
):
    """
...
def main():
    ...
    parser.add_argument(
        "--file", "-f", help="File containing statements (one per line)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model",
        default="cogito:32b",
        help="Ollama model to use (default: cogito:32b)",
    )
    ...
    if args.verbose:
        ...
        for i, result in enumerate(results, 1):
            ...
            print(
                f"   → {result['verdict']} ({result['topic']}) - Chunks: {result.get('chunks_retrieved', 'N/A')}"
            )
