...
def preprocess_document_content(content):
    ...
    content = re.sub(r'#{1,6}\s*Author Information and Affiliations.*?(?=\n#{1,6}\s|\Z)', '', content, flags=re.DOTALL | re.IGNORECASE)
    content = re.sub(r'#{1,6}\s*Authors?\s*\n.*?(?=\n#{1,6}\s|\Z)', '', content, flags=re.DOTALL)
    content = re.sub(r'#{1,6}\s*Affiliations?\s*\n.*?(?=\n#{1,6}\s|\Z)', '', content, flags=re.DOTALL)
    
    # Remove disclosure statements
    content = re.sub(r'\*\*Disclosure:\*\*.*?(?=\n|\Z)', '', content, flags=re.DOTALL)
    
    # Remove references section and everything after it
    content = re.sub(r'#{1,6}\s*References\s*\n.*', '', content, flags=re.DOTALL | re.IGNORECASE)
    
    # Remove review questions sections
    content = re.sub(r'#{1,6}\s*Review Questions.*?(?=\n#{1,6}\s|\Z)', '', content, flags=re.DOTALL | re.IGNORECASE)
    ...
    content = re.sub(r'\[Comment on this article.*?\]', '', content, flags=re.IGNORECASE)
        ...
        if topic_folder.is_dir():
            ...
            for file_path in md_files:
                ...
                try:
                    ...
                    file_topic = file_path.stem  # e.g., "ecg_t_wave" from "ecg_t_wave.md"
                    ...
                    for doc in docs:
                        ...
                        if len(cleaned_content.strip()) < 100:
                            print(f"  ⚠️ Skipping {file_path.name} - insufficient content after preprocessing")
                            continue
...
def calculate_chunk_ids(chunks: list[Document]):
    ...
    for i, chunk in enumerate(chunks):
        ...
        file_name = Path(source).name if source else chunk.metadata.get("source_file", "unknown")
...
def preview_preprocessing(sample_file=None):
    ...
    if sample_file:
        ...
        topic_name = file_path.parent.name if file_path.parent.name != 'data' else 'general'
        ...
        print(f"Reduction from original: {100 * (1 - len(processed)/len(original)):.1f}%")
        print(f"\nExtracted metadata:")
...
def main():
    ...
    parser.add_argument("--preview", type=str, help="Preview preprocessing on a sample file.")
