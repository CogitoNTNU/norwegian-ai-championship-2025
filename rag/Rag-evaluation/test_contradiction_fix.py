...
def test_contradiction_detection():
    ...
    statement_is_true, statement_topic = llm_client.classify_statement(
        statement, context
    )
...
def test_healthcare_rag_deduplication():
    ...
    if len(contexts) == len(unique_contexts):
        ...
        print(
            f"❌ FAILED: Found {len(contexts) - len(unique_contexts)} duplicate contexts"
        )
        return False
...
def test_full_pipeline():
    ...
    try:
        ...
        # statement_topic = answer.get("statement_topic", 0)  # Unused variable
