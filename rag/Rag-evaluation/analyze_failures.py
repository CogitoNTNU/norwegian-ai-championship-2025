...
def analyze_failures():
    ...
    print('Ground Truth Analysis: The statement is FALSE because the actual recommended threshold is >20-25%, not >15%.')
    ...
    for i, result in enumerate(results1['semantic_results']):
        doc = result['document']
        text = doc['text']
        print(f'  Result {i+1} | Score: {result["score"]:.2f} | Topic: {doc.get("topic_name")}')
        ...
        print(f'    -> Analysis: Contains >15%? {contains_15} | Contains >20%? {contains_20} | Mentions screening? {contains_screening}')
        if not contains_15 and not contains_20:
            print('    -> 🚨 FAILURE: Retrieved document is about MRI but lacks the specific risk percentage.')
        print()
    ...
    print("Ground Truth Analysis: The statement is FALSE. Takotsubo is characterized by the ABSENCE of necrosis and inflammation.")
    ...
    classified_topic = topic_bm25_results[0].get('classified_topic', 'Unknown') if topic_bm25_results else 'Unknown'
    ...
    for i, result in enumerate(topic_bm25_results):
        ...
        contains_absence = "absence of necrosis" in text.lower() or "without necrosis" in text.lower()
        print(f'    -> Analysis: Mentions necrosis? {contains_necrosis} | Mentions ABSENCE of necrosis? {contains_absence}')
        if not contains_absence:
            print('    -> 🚨 FAILURE: Retrieved document discusses the condition but does not contain the crucial contradictory evidence (the absence of necrosis).')
        print()
