...
def analyze_specific_examples():
    ...
    for i, case in enumerate(test_cases):
        ...
        semantic_context = "\n".join([doc['document']['text'] for doc in semantic_results])
        ...
        topic_bm25_results = diagnostic.topic_classified_bm25_retrieval(case["question"], k=5)
        topic_bm25_context = "\n".join([doc['document']['text'] for doc in topic_bm25_results])
        ...
        try:
            semantic_response = llm_client.classify_statement(case["question"], semantic_context)
            print(f"   Predicted: {semantic_response[0]} (Ground Truth: {case['ground_truth']})")
            print(f"   Correct: {'✅' if str(semantic_response[0]) == case['ground_truth'] else '❌'}")
        except Exception as e:
            print(f"   Error: {e}")
        
        print(f"\n🔑 BM25-ONLY LLM RESPONSE:")
        try:
            bm25_response = llm_client.classify_statement(case["question"], bm25_context)
            print(f"   Predicted: {bm25_response[0]} (Ground Truth: {case['ground_truth']})")
            print(f"   Correct: {'✅' if str(bm25_response[0]) == case['ground_truth'] else '❌'}")
        except Exception as e:
            print(f"   Error: {e}")
        ...
        try:
            ...
            if topic_bm25_results:
                ...
                print(f"   Classified Topic: {classified_topic} ({topic_doc_count} docs)")
            
            topic_bm25_response = llm_client.classify_statement(case["question"], topic_bm25_context)
            print(f"   Predicted: {topic_bm25_response[0]} (Ground Truth: {case['ground_truth']})")
            print(f"   Correct: {'✅' if str(topic_bm25_response[0]) == case['ground_truth'] else '❌'}")
        except Exception as e:
            print(f"   Error: {e}")
        
        print(f"\n🔄 HYBRID LLM RESPONSE:")
        try:
            hybrid_response = llm_client.classify_statement(case["question"], hybrid_context)
            print(f"   Predicted: {hybrid_response[0]} (Ground Truth: {case['ground_truth']})")
            print(f"   Correct: {'✅' if str(hybrid_response[0]) == case['ground_truth'] else '❌'}")
        except Exception as e:
            print(f"   Error: {e}")
        ...
        if topic_bm25_results:
            print(f"Classified Topic: {topic_bm25_results[0].get('classified_topic', 'Unknown')}")
            print(f"Topic Match: {'✅' if topic_bm25_results[0].get('classified_topic') == case['reference_contexts'][0] else '❌'}")
        print("="*80)
...
def analyze_random_sample():
    ...
    for i, case in enumerate(test_cases):
        ...
        # Test SmartRAG
        try:
            smart_rag_result = smart_rag.run(case["question"], case["reference_contexts"])
            smart_rag_answer = json.loads(smart_rag_result["answer"])
            smart_rag_correct = str(smart_rag_answer["statement_is_true"]) == case['ground_truth']
    ...
    print(f"{'='*60}")
    print(f"Semantic Accuracy:     {results['semantic_correct']}/{results['total']} = {results['semantic_correct']/results['total']:.2%}")
    print(f"BM25 Accuracy:         {results['bm25_correct']}/{results['total']} = {results['bm25_correct']/results['total']:.2%}")
    print(f"Topic-BM25 Accuracy:   {results['topic_bm25_correct']}/{results['total']} = {results['topic_bm25_correct']/results['total']:.2%}")
    print(f"🚀 SmartRAG Accuracy:  {results['smart_rag_correct']}/{results['total']} = {results['smart_rag_correct']/results['total']:.2%}")
    print(f"Hybrid Accuracy:       {results['hybrid_correct']}/{results['total']} = {results['hybrid_correct']/results['total']:.2%}")

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="Debug BioBERT retrieval issues")
    parser.add_argument("--mode", choices=["specific", "random"], default="specific",
