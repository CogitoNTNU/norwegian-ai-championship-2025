...
class BiobertFast:
    ...
    def __init__(self, llm_client):
        ...
        self.topic_classifier = SentenceTransformer(
    ...
    def _load_optimized_indexes(self):
        ...
        print(
            f"   • {len(self.documents)} documents from {len(self.topic_mapping)} topics"
        )
    ...
    def _compute_topic_embeddings(self):
        ...
        self.topic_embeddings = self.topic_classifier.encode(
            self.topic_names, batch_size=32, show_progress_bar=False
        )
    ...
    def retrieve_documents(self, query: str, top_k: int = 5) -> List[str]:
        ...
        results, scores = retriever.retrieve(
            query_tokens, k=min(top_k, len(relevant_docs))
        )
        ...
        if total_time > 2.0:  # Only log if retrieval is slow
            print(
                f"Retrieval timing: topic={topic_time:.2f}s, docs={docs_time:.2f}s, bm25={retrieval_time:.2f}s, total={total_time:.2f}s"
            )

        return contexts

    def run(
        self, user_input: str, reference_contexts: List[str] = None
    ) -> Dict[str, Any]:
...
Response:"""
        ...
        try:
            ...
            if total_time > 5.0:
                print(
                    f"⚠️ Slow response: retrieval={retrieval_time:.2f}s, llm={llm_time:.2f}s, total={total_time:.2f}s"
                )

            return {"answer": response, "context": retrieved_contexts}
