...
class HybridRAGAppleSilicon:
    ...
    def _load_optimized_indexes(self):
        ...
        if not optimized_dir.exists():
            raise FileNotFoundError(f"Optimized indexes directory not found: {optimized_dir}")
        ...
        if biobert_faiss_path.exists():
            ...
            print(f"   ✅ Loaded BioBERT FAISS index ({biobert_faiss_path.stat().st_size / 1024 / 1024:.1f}MB)")
        ...
        if bm25_path.exists():
            ...
            print(f"   ✅ Loaded global BM25 index ({bm25_path.stat().st_size / 1024 / 1024:.1f}MB)")
        ...
        if mapping_path.exists():
            ...
            print(f"   ✅ Loaded document mapping ({len(self.documents)} docs, {mapping_path.stat().st_size / 1024 / 1024:.1f}MB)")
    ...
    def _safe_faiss_search(self, query_embeddings: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def run(self, question: str, reference_contexts: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            context = "\n\n".join([f"[{doc.get('article_title', 'Medical Text')}]: {doc['text']}" for doc in candidate_docs])
            
            # Use LLM client for classification (no topic override)
            statement_is_true, statement_topic = self.llm_client.classify_statement(question, context)
