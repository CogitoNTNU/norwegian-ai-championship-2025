...
class GraphRAG:
    ...
    def __init__(self, llm_client: LocalLLMClient = None):
        ...
        self.medical_graph = {
            "symptoms": ["fever", "cough", "chest pain", "shortness of breath", "nausea"],
            "conditions": ["pneumonia", "flu", "covid-19", "heart attack", "appendicitis"],
            "treatments": ["antibiotics", "rest", "oxygen therapy", "surgery", "medication"],
            "relationships": {
    ...
    def retrieve_context(self, question: str, reference_contexts: List[str], k: int = 5) -> List[str]:
            ...
            for symptom in self.medical_graph["symptoms"]:
                if symptom in question_lower:
                    ...
                    for condition in related:
                        relevant_contexts.append(f"Medical knowledge: {symptom} is associated with {condition}")
            
            # Add reference contexts if provided
            if reference_contexts:
                relevant_contexts.extend(reference_contexts[:k-len(relevant_contexts)])
