...
class BiobertDiagnostic:
    ...
    def _load_and_embed_data(self):
        ...
        if all(
            os.path.exists(p) for p in [faiss_index_path, bm25_index_path, mapping_path]
        ):
            ...
            if os.path.exists(metadata_path):
                ...
                print("✅ Loaded optimized indexes:")
                print(
                ...
                print(
                    f"✅ Loaded {len(self.documents)} documents from optimized indexes."
                )
        else:
            ...
            chunks_file_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "chunking", "kg", "chunks.jsonl"
            )
            ...
            if self.documents:
                ...
                self.embeddings = self.embedding_model.encode(
                    self.document_texts, show_progress_bar=True
                )
    ...
    def semantic_only_retrieval(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        return [
            {"document": self.documents[idx], "score": distances[0][i], "rank": i + 1}
    ...
    def bm25_only_retrieval(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        return [
            {"document": self.documents[idx], "score": score, "rank": i + 1}
    ...
    def _generate_contradictory_hypothesis(self, query: str) -> str:
        ...
        # In a real system, you might use a prompt like:
        # f"Please generate a single, concise sentence that directly contradicts the following statement: \"{query}\""
    ...
    def _rerank_with_nli(
        ...
        for result in results:
            doc_id = result["document"]["chunk_id"]
            result["nli_contradiction_score"] = doc_to_max_contradiction_score.get(
                doc_id, -1
            )

        return sorted(results, key=lambda x: x["nli_contradiction_score"], reverse=True)

    def topic_classified_bm25_retrieval(
        ...
        distances, indices = self.index.search(
            query_embedding.astype("float32"), 20
        )  # Get top 20 for topic classification and reranking
        ...
        query_tokens = self._tokenize_and_stem(query)
        try:
            bm25_results_data = topic_bm25_index.retrieve(
                [query_tokens], k=k * 2
            ...
            results = []
            for i, (topic_idx, score) in enumerate(
                zip(bm25_results_indices, bm25_scores)
            ):
                result = {
            ...
            if reranked_results:
                reranked_results[0]["score"] *= (
                    2.0  # Double the score of the top NLI result
                )
    ...
    def run(
        ...
        if not retrieved_docs:
            return {
                "answer": json.dumps(
                    {"statement_is_true": False, "statement_topic": 0}
                ),
                "context": [],
            }
                ...
                "answer": json.dumps(
                    {
                        ...
                        "statement_topic": self._topic_name_to_number(
                            self.classify_topic(retrieved_docs)
                        ),
        ...
        # prompt = f"""
        # You are a clinical fact-checker. Your task is to verify a medical statement against a provided context document and classify …
        #
        # **CONTEXT DOCUMENT:**
        # {context}
        #
    ...
    def run_all_retrievers(
        ...
        topic_classified_bm25_results = self.topic_classified_bm25_retrieval(
            question, k=5
        )
    ...
    def analyze_question(
        self, question: str, ground_truth: str, reference_contexts: List[str] = None
    ):
        ...
        print(
            f"REFERENCE TOPIC: {reference_contexts[0] if reference_contexts else 'Unknown'}"
        )
        print(f"{'=' * 80}")
        ...
        for i, result in enumerate(diagnostic_data["hybrid_results"]):
            ...
            print(
                f"     Semantic Rank: {result['semantic_rank']} (contrib: {result['semantic_contribution']:.4f})"
            )
            print(
                f"     BM25 Rank: {result['bm25_rank']} (contrib: {result['bm25_contribution']:.4f})"
            )
            print(f"     Topic: {doc.get('topic_name', 'Unknown')}")
            print(f"     Text: {doc['text'][:200]}...")
            print()
        ...
        for i, result in enumerate(diagnostic_data["topic_classified_bm25_results"]):
            ...
            print(f"  {i + 1}. Score: {result['score']:.4f}")
            print(
                f"     Classified Topic: {result.get('classified_topic', 'Unknown')} ({result.get('topic_doc_count', 0)} docs)"
            )
            print(f"     Topic: {doc.get('topic_name', 'Unknown')}")
            print(f"     Text: {doc['text'][:200]}...")
            print()
...
class BiobertTopicBM25(BiobertDiagnostic):
    ...
    def run(
        ...
        statement_is_true, _ = self.llm_client.classify_statement(
