...
def create_testset_from_split_data(test_dir: str, project_root: str) -> list[dict]:
...
def parse_ground_truth(ground_truth_str: str) -> tuple[int, int]:
    """
    Parse ground truth string (simple "1" or "0" format for binary classification)

    Args:
        ground_truth_str: String like "1" (true) or "0" (false)

    Returns:
        Tuple of (statement_is_true, topic) - topic will be extracted from reference_contexts separately
    """
    try:
...
def main():
    ...
    NUM_COMPONENTS = 115  # Number of medical topics
    ...
    for template_name, results in template_results.items():
        ...
        # Load topic mapping for evaluation
        topics_path = os.path.join(project_root, "data", "topics.json")
        try:
            with open(topics_path, "r") as f:
        ...
        for item in results:
            ...
            if item["reference_contexts"] and len(item["reference_contexts"]) > 0:
                topic_name = item["reference_contexts"][0]
                gt_topic = topic_mapping.get(topic_name, 0)
