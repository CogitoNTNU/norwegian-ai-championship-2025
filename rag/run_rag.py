...
def main():
    ...
    parser.add_argument(
        "--statement", type=str, required=True, help="Medical statement to classify"
    )
    parser.add_argument(
        "--embedding",
        type=str,
    ...
    print("🏥 Medical Statement RAG Pipeline")
    print("=" * 50)
    ...
    try:
        ...
        print("\n✅ Results:")
        print(
            f"   Statement is true: {statement_is_true} ({'True' if statement_is_true else 'False'})"
        )
        ...
        if topics_json.exists():
            ...
            with open(topics_json) as f:
                topics = json.load(f)
            topic_name = next(
                (name for name, id in topics.items() if id == statement_topic),
                "Unknown",
            )
