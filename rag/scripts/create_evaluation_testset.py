...
def create_evaluation_testset():
    ...
    for i, entry in enumerate(testset[:3]):
        ...
        print(f"      Reference: {entry['reference']} (topic: {entry['reference_contexts'][0]})")
...
def main():
    ...
    try:
        testset_path = create_evaluation_testset()
        print(f"\n✅ Evaluation testset created successfully!")
        print(f"   The rag-evaluation system will now use the proper 15% test split")
        print(f"   No data leakage: evaluation uses only held-out test data")
