...
def load_combined_data() -> List[Tuple[str, str, dict]]:
    ...
    if not statements_dir.exists() or not answers_dir.exists():
        raise FileNotFoundError("Combined training data not found. Run combine_datasets.py first.")
...
def split_data(data: List[Tuple[str, str, dict]], train_ratio: float = 0.85) -> Tuple[List, List]:
...
def analyze_split(train_data: List, test_data: List):
    ...
    print(f"  Topics only in training: {len(set(train_topics.keys()) - set(test_topics.keys()))}")
    print(f"  Topics only in test: {len(set(test_topics.keys()) - set(train_topics.keys()))}")
...
def main():
    ...
    print(f"  All methods will use only the training portion (85%)")
