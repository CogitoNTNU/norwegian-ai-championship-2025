...
JSON:"""
        ...
        return {"keywords": medical_words, "questions": [f"What is {statement}?"]}
...
class HybridRetriever:
    ...
    def __init__(self, k_sparse: int = config.K_SPARSE, k_dense: int = config.K_DENSE):
        ...
        self.embedder = SentenceTransformer(config.EMBEDDER_MODEL, device=self.device)
    ...
    def _ensure_indices(self) -> Tuple[Any, Any, pd.DataFrame]:
        ...
        if (
            config.BM25_CACHE.exists()
            and config.FAISS_CACHE.exists()
            and config.META_CACHE.exists()
        ):
    ...
    def _build_indices(self) -> Tuple[Any, Any, pd.DataFrame]:
        ...
        metadata = pd.DataFrame(
            {"id": range(len(texts)), "text": texts, "topic_id": topic_ids}
        )

        # Build BM25s index
        logger.info("Building BM25s index...")
        tokenized = [
            [token for token in re.findall(r"\b\w+\b", text.lower())] for text in texts
        ]
                ...
                results.append(
                    {
                        ...
                        "score": float(
                            scores[0][pos]
                        ),  # scores[query_index][result_index]
                        "source": "bm25",
                    }
                )
        return results
...
class QwenReranker:
    ...
    def __init__(self, model_name: str = config.RERANKER_MODEL):
        ...
        if config.USE_LIGHT_RERANKER:
            self.model = CrossEncoder(
                "mixedbread-ai/mxbai-rerank-base-v1", device=self.device
            )
            self.tokenizer = None
        else:
    ...
    async def rerank(
        self, query: str, documents: List[Dict[str, Any]], top_k: int = config.K_FINAL
    ) -> List[Dict[str, Any]]:
        ...
        if config.USE_LIGHT_RERANKER:
            ...
            pairs = [
                (query, doc["text"][: config.MAX_CONTENT_LENGTH])
...
class StatementClassifier:
    ...
    async def classify(
        self, statement: str, context_docs: List[Dict[str, Any]]
    ) -> Dict[str, int]:
        ...
        for keyword, topic_ids in domain_keywords.items():
            ...
            if keyword in primary_text or any(
...
JSON (required format):"""
    ...
    async def _try_classification(self, prompt: str) -> Optional[Dict[str, int]]:
        ...
        try:
            ...
            logger.warning(
                f"All JSON extraction methods failed for content: {content[:200]}..."
            )
            return None
    ...
    def _validate_result(self, result: Dict) -> Optional[Dict[str, int]]:
        ...
        try:
            if (
                ...
                and "statement_is_true" in result
                and "statement_topic" in result
                and isinstance(result["statement_is_true"], int)
                and isinstance(result["statement_topic"], int)
                and 0 <= result["statement_is_true"] <= 1
                and 0 <= result["statement_topic"] <= 114
            ):
                return result
        except Exception:
            pass
