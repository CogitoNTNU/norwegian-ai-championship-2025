...
class DocumentManager:
    ...
    def load_or_build_chunks(
        self, topics_dir: str, topics_json: str, force_rebuild: bool = False
        ...
        if (
            ...
            and self.chunks_hash_file.exists()
        ):
        ...
        with open(self.chunks_file, "w") as f:
            json.dump(
                {"chunks": self.chunks, "metadata": self.chunk_metadata}, f, indent=2
            )
    ...
    def get_chunk_by_indices(self, indices: List[int]) -> List[Dict[str, Any]]:
        ...
        for idx in indices:
            if 0 <= idx < len(self.chunks):
                results.append(
                    {"chunk": self.chunks[idx], "metadata": self.chunk_metadata[idx]}
                )
