...
def get_sensor_data_from_observation(observation: np.ndarray) -> dict:
    ...
    # Extract velocity
    vel_x = float(observation[num_sensors] * 30.0)  # Denormalize and convert to Python float
...
def watch_bot_in_ppo_env(episodes: int = 5, render_speed: float = 1.0, verbose: bool = True):
    """
    ...
    try:
        for episode in range(episodes):
            ...
            episode_reward = 0
            episode_steps = 0
            action_counts = {"NOTHING": 0, "ACCELERATE": 0, "DECELERATE": 0, "STEER_LEFT": 0, "STEER_RIGHT": 0}
            ...
            while not done:
                try:
                    ...
                    if current_action_queue:
                        ...
                        action_str = current_action_queue.pop(0) if current_action_queue else "NOTHING"
        ...
        if episode_stats:
            ...
            print(f"Average Distance: {np.mean(distances):.1f} ± {np.std(distances):.1f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Watch bot_with_memory in PPO environment")
