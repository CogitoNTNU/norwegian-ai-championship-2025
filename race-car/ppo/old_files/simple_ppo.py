            ...
            for i, done in enumerate(self.locals["dones"]):
                if done:
                    ...
                    if "episode" in info:
                        ...
                        wandb.log(
                            {
                                ...
                                "episode/avg_distance_last_100": np.mean(
                                    self.episode_distances[-100:]
                                )
                                if self.episode_distances
                                else 0,
                                "episode/avg_reward_last_100": np.mean(
                        ...
                        if self.verbose > 0:
                            print(
                                f"Episode finished - Distance: {distance:.1f}, Reward: {reward:.1f}, Length: {length}"
                            )
...
def make_env():
    ...
    env = RecordVideo(
        env,
        f"videos/{run.id}",
        episode_trigger=lambda episode_id: episode_id % 10
        == 0,  # Record every 10th episode
