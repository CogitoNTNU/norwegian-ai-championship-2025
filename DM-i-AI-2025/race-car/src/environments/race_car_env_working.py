...
class RaceCarEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Handle collisions (missing from update_game function)
        for car in self.state.cars:
            if car != self.state.ego and core.intersects(self.state.ego.rect, car.rect):
                self.state.crashed = True

        # Check collision with walls
        for wall in self.state.road.walls:
            if core.intersects(self.state.ego.rect, wall.rect):
                self.state.crashed = True

        terminated = (
            self.state.crashed
            or self.state.ticks > MAX_TICKS
            or self.state.elapsed_game_time > MAX_MS
        )
    ...
    def _get_obs(self):
        return np.array(
            [
                (sensor.reading / 1000.0) if sensor.reading is not None else 1.0
                for sensor in self.state.sensors
            ],
            dtype=np.float32,
        )
