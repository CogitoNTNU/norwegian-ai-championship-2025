...
blank_trans = (0, np.zeros((20,), dtype=np.float32), 0, 0.0, False)
...
class SegmentTree:
    def __init__(self, size):
        ...
        self.data = np.array(
            [blank_trans] * size, dtype=Transition_dtype
...
class ReplayMemory:
    ...
    def append(self, state, action, reward, terminal):
        ...
        )  # Only store last state vector
        self.transitions.append(
            (self.t, state.numpy(), action, reward, not terminal), self.transitions.max
    ...
    def _get_samples_from_segments(self, batch_size, p_total):
        ...
        # Create states and nth next states
        all_states = np.array(transitions["state"], copy=True)
        states = torch.tensor(
        ...
        actions = torch.tensor(
            np.array(transitions["action"][:, self.history - 1], copy=True),
            dtype=torch.int64,
            device=self.device,
        )
        ...
        rewards = torch.tensor(
            np.array(transitions["reward"][:, self.history - 1 : -1], copy=True),
            dtype=torch.float32,
            device=self.device,
        )
    ...
    def __next__(self):
        ...
        transitions[blank_mask] = blank_trans
        state = torch.tensor(
            transitions["state"].copy(), dtype=torch.float32, device=self.device
